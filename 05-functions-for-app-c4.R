## functions for multinomial numerical study, where Algorithm 1 is used
## instead of Algorithm 3
...
targetPower <- function(u, params, delta, n_val, q, hyper){
  ...
  ## generate maximum likelihood estimates on the logit(Z) scale
  lz11 <- qnorm(u[1], log(p.1.1) - log(1-p.1.1), 1/sqrt(n_val*p.1.1*(1-p.1.1)))
  lz12 <- qnorm(u[2], log(z.1.2) - log(1-z.1.2), 1/sqrt(n_val*p.1.2*(1-z.1.2)))
  lz13 <- qnorm(u[3], log(z.1.3) - log(1-z.1.3), 1/sqrt(n_val*p.1.3*(1-z.1.3)))
  lz14 <- qnorm(u[4], log(z.1.4) - log(1-z.1.4), 1/sqrt(n_val*p.1.4*(1-z.1.4)))
  
  lz21 <- qnorm(u[5], log(p.2.1) - log(1-p.2.1), 1/sqrt(round(q*n_val)*p.2.1*(1-p.2.1)))
  lz22 <- qnorm(u[6], log(z.2.2) - log(1-z.2.2), 1/sqrt(round(q*n_val)*p.2.2*(1-z.2.2)))
  lz23 <- qnorm(u[7], log(z.2.3) - log(1-z.2.3), 1/sqrt(round(q*n_val)*p.2.3*(1-z.2.3)))
  lz24 <- qnorm(u[8], log(z.2.4) - log(1-z.2.4), 1/sqrt(round(q*n_val)*p.2.4*(1-z.2.4)))
  
  ## convert maximum likelihood estimates to Z-scale for each group
  z11 <- 1/(1+exp(-lz11))
  z12 <- 1/(1+exp(-lz12))
  z13 <- 1/(1+exp(-lz13))
  z14 <- 1/(1+exp(-lz14))
  ...
  ## convert back to the p-scale to obtain sufficient statistics
  p12 <- z12*(1-z11)
  p13 <- z13*(1-z12)*(1-z11)
  ...
  ## generate a sample of size n_val using the sufficient statistics
  N1 <- n_val; N2 <- round(q*n_val)
  dat1 <- N1*c(z11, p12, p13, p14, 1 - z11 - p12 - p13 - p14)
  dat2 <- N2*c(z21, p22, p23, p24, 1 - z21 - p22 - p23 - p24)
  
  ## the remainder of Algorithm 1 is the same as Algorithm 3
...
## The next two functions are the same as in 03-function-for-sec-4-5.R; we have
## repeated them so as to keep the functions for this numerical study self-contained
...
## this helper function implements Algorithm 3 with the illustrative example to
## return the logit of the posterior probability; this is the same function
## as provided at the beginning of the 03-function-for-sec-4-5.R file.
## It will be used to illustrate the quality of the linear approximations
## on the logit scale
targetPowerAlg3 <- function(u, params, delta, n_val, q, hyper){
...
## this helper function implements an alternative to Algorithm 3 that leverages
## the exact (discrete) binomial distributions that comprise the multinomial
## model; it will be used to illustrate the quality of the linear approximations
## on the logit scale
targetPowerDiscrete <- function(u, params, delta, n_val, q, hyper){
  ...
  dat1 <- rep(0, 5); dat2 <- rep(0, 5); N1 <- n_val
  
  ## use the binomial models with the Z variables to get a multinomial sample
  dat1[1] <- qbinom(u[1], n_val, p.1.1)
  dat1[2] <- qbinom(u[2], n_val - ceiling(dat1[1]), z.1.2)
  dat1[3] <- qbinom(u[3], n_val - ceiling(sum(dat1[1:2])), z.1.3)
  dat1[4] <- qbinom(u[4], n_val - ceiling(sum(dat1[1:3])), z.1.4)
  dat1[5] <- n_val - sum(dat1[1:4])
  
  ## repeat for group 2
  N2 <- round(q*n_val)
  dat2[1] <- qbinom(u[5], N2, p.2.1)
  dat2[2] <- qbinom(u[6], N2 - ceiling(dat2[1]), z.2.2)
  dat2[3] <- qbinom(u[7], N2 - ceiling(sum(dat2[1:2])), z.2.3)
  dat2[4] <- qbinom(u[8], N2 - ceiling(sum(dat2[1:3])), z.2.4)
  dat2[5] <- N2 - sum(dat2[1:4])
  
  ## The remainder of the process is the same as Algorithm 3
  ## extract hyperparameters
  alphas1 <- hyper[,1]
  betas1 <- hyper[,2]
  alphas2 <- hyper[,3]
